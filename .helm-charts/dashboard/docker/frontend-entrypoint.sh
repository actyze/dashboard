...
if [ -n "$REACT_APP_GA_TRACKING_ID" ]; then
  ...
  # Replace placeholder with actual GA scripts
  sed -i "s|<!-- GA_TRACKING_PLACEHOLDER - Replaced at container startup by deployment scripts -->|${GA_SCRIPTS}|g" "$INDEX_HTML"
  ...
  sed -i "s|<!-- GA_TRACKING_PLACEHOLDER - Replaced at container startup by deployment scripts -->||g" "$INDEX_HTML"
fi
