...
const DashboardsList = () => {
  ...
  const getFilteredAndSortedDashboards = () => {
    ...
    if (statusFilter !== "all") {
      filtered = filtered.filter(
        (dashboard) => getStatus(dashboard) === statusFilter,
      );
    }
  ...
  return (
    <div
      className={`h-full flex flex-col ${isDark ? "bg-[#101012]" : "bg-white"}`}
    >
      {/* Header */}
      <div
        className={`px-6 py-4 flex items-center justify-between border-b ${isDark ? "border-[#2a2b2e]" : "border-gray-200"}`}
      >
      ...
      <div className="flex-1 overflow-auto">
        {/* Table Header */}
        <div
          className={`grid grid-cols-12 gap-4 px-6 py-2 text-xs font-medium border-b sticky top-0 ${
            isDark
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center h-64 gap-4">
            ...
            <p className={`text-sm text-center max-w-md ${isDark ? "text-gray-500" : "text-gray-400"}`}>
              Create your first dashboard to visualize and track your key
              metrics.
            </p>
          ...
          <div className="flex flex-col items-center justify-center py-16">
            <svg
              className={`w-8 h-8 mb-2 ${isDark ? "text-gray-600" : "text-gray-400"}`}
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
            ...
            {sortedDashboards.map((dashboard) => (
              ...
              >
                ...
                <div className="col-span-8 flex items-center min-w-0">
                  <span
                    className={`truncate text-sm ${isDark ? "text-gray-200" : "text-gray-900"}`}
                  >
                    {dashboard.title}
                  </span>
                </div>
